#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time : 2021/10/14 20:58
# @Author : Jiaqi Guo
# @Site : 
# @File : class_incremental_cosine_tbal_cub200.py
# @Software: PyCharm
...
import dataset_zzy_ as datasets
import core.tbal_net as tbalnet

if __name__ == '__main__':

    ######### Modifiable Settings ##########
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataset', default='seed_1993_subset_100_CubBirds', type=str)
    parser.add_argument('--datadir', default='data/seed_1993_subset_100_imagenet/data', type=str)
    parser.add_argument('--saved_dir', default='CUB200_Gaborresnet50_Cosine', type=str)
    ...
    parser.add_argument('--resume', default=1, type=float, \
        help='resume from checkpoint')
    ...
    parser.add_argument('--imprint_weights', default=1, type=float, \
                        help='Imprint the weights for novel classes')
    # parser.add_argument('--less_forget', action='store_true', \
    #     help='Less forgetful')
    parser.add_argument('--less_forget', default=1, type=float, \
        help='Less forgetful')
    ...
    parser.add_argument('--adapt_lamda', default=1, type=float, \
        help='Adaptively change lamda')
    ...
    parser.add_argument('--mr_loss', default=1, type=float, \
        help='Margin ranking loss v1')
    parser.add_argument('--amr_loss', default=0, type=float, \
        help='Margin ranking loss v2')
    ...
    ########################################
    assert(args.nb_cl_fg % args.nb_cl == 0) # 这里必须保证初始的训练集类别数量可以整除每次增加的新类别数量
    assert(args.nb_cl_fg >= args.nb_cl)
    train_batch_size       = 8            # Batch size for train
    test_batch_size        = 8             # Batch size for test
    eval_batch_size        = 8            # Batch size for eval
    ...
    # Data loading code
    trainset = datasets.CUB(root='C:/zhaozy/datasets/FGVC/CUB_200_2011/CUB_200_2011', is_train=True, data_len=None)
    testset = datasets.CUB(root='C:/zhaozy/datasets/FGVC/CUB_200_2011/CUB_200_2011', is_train=False, data_len=None)
    evalset = datasets.CUB(root='C:/zhaozy/datasets/FGVC/CUB_200_2011/CUB_200_2011', is_train=False, data_len=None)

    # Initialization
    dictionary_size     = args.nb_protos * args.num_classes #
    ...
    X_train_total, Y_train_total = np.array(trainset.train_img), np.array(trainset.train_label)
    X_valid_total, Y_valid_total = np.array(testset.test_img), np.array(testset.test_label)

    saved_dir = args.saved_dir
    saved_dir = os.path.join(saved_dir, 'checkpoint')
    ...
    for iteration_total in range(args.nb_runs):
        ...
        order_name = "seed_{}_{}_order_run_{}.pkl".format(args.random_seed, args.dataset, iteration_total)
        order_name = os.path.join(saved_dir, order_name)
        ...
        for orde in range(args.num_classes):
            ...
            # prototypes中以类别为索引，存储了所有训练样本，需要注意的是，这里的排序方式是随机排列之后的顺序
        ...
        for iteration in range(start_iter, int(args.num_classes/args.nb_cl)):
            ...
            if iteration == start_iter:
                ...
                last_iter = 0
                ############################################################
                tg_model = tbalnet.TBAL_Net(num_classes=args.nb_cl_fg)
                ...
                # new_fc.sigma.data = tg_model.fc.sigma.data # 这里应该有错误，fc的原始版本里并没有sigma这个参数
                tg_model.fc = new_fc
            ...
            if iteration == start_iter:
                X_valid_ori = X_valid # 在第一次训练中，测试集合就是first group中的类别
                Y_valid_ori = Y_valid
            else:
                ...
                # 在第一次循环之后，X_train为当前的新类别数据与之前选择过后的旧类别样本的集合
            ...
            if iteration > start_iter and args.imprint_weights:
                ...
                for cls_idx in range(iteration * args.nb_cl, (iteration+1) * args.nb_cl):
                    ...
                    evalset.test_img = X_train[cls_indices]
                    evalset.test_label = np.zeros(len(X_train[cls_indices]))
            ...
            trainset.train_img = X_train
            trainset.train_label = map_Y_train
            ...
            '''
            这里需要注意，每次测试集合包含的类别数量，是指目前为止的所有类别数
            '''
            testset.test_img = X_valid_cumul
            testset.test_label = map_Y_valid_cumul
            ...
            ##############################################################
            ckp_name = '{}_run_{}_iteration_{}_model.pth'.format(args.ckp_prefix, iteration_total, iteration)
            ckp_name = os.path.join(saved_dir, ckp_name)
            ...
            if args.resume and os.path.exists(ckp_name):
                ...
                if iteration > start_iter and args.less_forget:
                    ...
                    map函数将第二个参数表达的一系列参数，带入第一个参数表达的函数
                    '''
                    ...
                    if args.less_forget:
                        ...
                        if args.mimic_score:
                            ...
                            tg_model = incremental_train_and_eval(args.epochs, tg_model, ref_model, tg_optimizer, tg_lr_scheduler, \
                                trainloader, testloader, \
                                iteration, start_iter,
                                args.T, args.beta, device=device)
                if not os.path.isdir('checkpoint'):
                    os.mkdir('checkpoint')
                torch.save(tg_model, ckp_name)

            ### Exemplars
            if args.fix_budget:# 指出大小是否固定
            ...
            for iter_dico in range(last_iter*args.nb_cl, (iteration+1)*args.nb_cl):
                ...
                evalset.test_img = prototypes[iter_dico]
                evalset.test_label = np.zeros(len(prototypes[iter_dico]))
                evalloader = torch.utils.data.DataLoader(evalset, batch_size=eval_batch_size,
                ...
                '''
                计算每个类别的特征，并且将其记录在张量中，mapped_prototypes的size为【num_samples, num_features】，
                num_features是输入到fc的特征维度
                '''
                D = mapped_prototypes.T
                D = D/np.linalg.norm(D, axis=0) # 对每个特征向量进行归一化

                # Herding procedure : ranking of the potential exemplars
                mu = np.mean(D, axis=1) # size为【num_features,】,计算出了当前类别的均值向量
                ...
                while not (np.sum(alpha_dr_herding[index1, :, index2] != 0) == min(nb_protos_cl, 500)) and iter_herding_eff < 1000:
                    ...
                    这里
                    '''
                    tmp_t = np.dot(w_t, D) # w_t的size是[num_features,]， D的size为[num_features, num_samples]
                                        # 根据我的理解，这个地方可以理解为余弦相似度，完全一致为1，完全相反为-1
                    ind_max = np.argmax(tmp_t) # 输出的tmp_t的size为【num_samples】，求得了与均值最相似的特征向量
                    iter_herding_eff += 1
                    if alpha_dr_herding[index1, ind_max, index2] == 0:
                        alpha_dr_herding[index1, ind_max, index2] = 1 + iter_herding
                        iter_herding += 1
                    w_t = w_t+mu-D[:, ind_max] # D的size【num_features, num_samples】
            ...
            # class_means = np.zeros((64,100,2)) # 这个是对cifar100数据库来说的
            ...
            for iteration2 in range(iteration+1):
                for iter_dico in range(args.nb_cl):
                    ...
                    evalset.test_img = prototypes[iteration2 * args.nb_cl + iter_dico]
                    evalset.test_label = np.zeros(len(prototypes[iteration2*args.nb_cl+iter_dico]))
                    evalloader = torch.utils.data.DataLoader(evalset, batch_size=eval_batch_size,
                        shuffle=False, num_workers=args.num_workers, pin_memory=True)
            ...
            mean_file = '{}_run_{}_iteration_{}_class_means.pth'.format(args.ckp_prefix, iteration_total, iteration)
            mean_file = os.path.join(saved_dir, mean_file)
            torch.save(class_means, mean_file)
            ...
            evalset.test_img = X_valid_ori
            evalset.test_label = map_Y_valid_ori
            evalloader = torch.utils.data.DataLoader(evalset, batch_size=eval_batch_size,
                    shuffle=False, num_workers=args.num_workers, pin_memory=True)
            ...
            evalset.test_img = X_valid_cumul
            evalset.test_label = map_Y_valid_cumul
            evalloader = torch.utils.data.DataLoader(evalset, batch_size=eval_batch_size,
                    shuffle=False, num_workers=args.num_workers, pin_memory=True)
        ...
        acc_ori_file = '{}_run_{}_top1_acc_list_ori.pth'.format(args.ckp_prefix, iteration_total)
        acc_ori_file = os.path.join(saved_dir, acc_ori_file)
        acc_cumul_file = '{}_run_{}_top1_acc_list_cumul.pth'.format(args.ckp_prefix, iteration_total)
        acc_cumul_file = os.path.join(saved_dir, acc_cumul_file)

        torch.save(top1_acc_list_ori, acc_ori_file)
        torch.save(top1_acc_list_cumul, acc_cumul_file)