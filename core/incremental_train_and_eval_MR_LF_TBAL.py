...
# @Author : Jiaqi Guo
# @Site : 
# @File : incremental_train_and_eval_MR_LF_TBAL.py
# @Software: PyCharm
...
def incremental_train_and_eval_MR_LF_TBAL(epochs, tg_model, ref_model, tg_optimizer, tg_lr_scheduler, \
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            backbone_outputs, local_outputs, _ = tg_model(inputs)
            if iteration == start_iteration:
                loss_1 = nn.CrossEntropyLoss(weight_per_class)(backbone_outputs, targets)
                loss_2 = nn.CrossEntropyLoss(weight_per_class)(local_outputs, targets.unsqueeze(1).repeat(1, tg_model.proposalN).view(-1))
                loss = loss_1 + loss_2
            else:
                ref_backbone_outputs, ref_local_outputs, _ = ref_model(inputs)
                loss1 = nn.CosineEmbeddingLoss()(cur_features, ref_features.detach(), \
                    torch.ones(inputs.shape[0]).to(device)) * lamda
                loss2 = nn.CrossEntropyLoss(weight_per_class)(ref_backbone_outputs, targets)
