...
def compute_accuracy(tg_model, tg_feature_model, class_means, evalloader, scale=None, print_info=True, device=None):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(evalloader):
            ...
            # tt = tg_feature_model(inputs).cpu()
            # print(tt.size())
            outputs_feature = np.squeeze(tg_feature_model(inputs).cpu(), axis=-1)
            outputs_feature = np.squeeze(outputs_feature, axis=-1)

            # Compute score for iCaRL
            # print(class_means.shape)
            # print(outputs_feature.size())
