...
# @Author : Jiaqi Guo
# @Site : 
# @File : class_incremental_cub200.py
# @Software: PyCharm
...
if __name__ == '__main__':
    ...
    train_batch_size       = 16            # Batch size for train
    test_batch_size        = 16             # Batch size for test
    eval_batch_size        = 16            # Batch size for eval
    ...
    # X_train_total, Y_train_total = split_images_labels(trainset.train_img)
    # X_valid_total, Y_valid_total = split_images_labels(testset.test_img)
    ...
    for iteration_total in range(args.nb_runs):
        ...
        if os.path.exists(order_name):
            ...
            order = np.arange(args.num_classes) # 打乱类别
        ...
        '''
        说一句nb_runs的作用，作为将一个固定的数据库修改为类别逐渐增加的过程，哪些类别放在前面，是否会对结果产生影响？
        为了解释这个问题，需要多运行几次实验，每次实验都随机生成一个order，将各个类别进行随机排列
        '''
        for iteration in range(start_iter, int(args.num_classes/args.nb_cl)):
            ...
            if iteration == start_iter:
                ...
                # tg_model = models.resnet18(pretrained=True, num_classes=args.nb_cl_fg)
                ref_model = None
            else:
                ############################################################
                last_iter = iteration
            ...
            X_train          = X_train_total[indices_train_10] # 当前迭代中的训练数据
            ...
            # trainset.train_img = trainset.samples = current_train_imgs
            ...
            # testset.test_img = testset.samples = current_test_images
            ...
            for iter_dico in range(last_iter*args.nb_cl, (iteration+1)*args.nb_cl):
                ...
                # evalset.test_img = evalset.samples = current_eval_set

                evalset.test_img = prototypes[iter_dico]
                evalset.test_label = np.zeros(len(prototypes[iter_dico])) # 这里只是计算特征，图像的label不重要，没有用到
                ...
                # mapped_prototyoes是一个[num_samples, num_features]的矩阵
